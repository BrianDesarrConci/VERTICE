    ...
    <>
      ...
      <section className="container pt-8">
        <div className="relative overflow-hidden rounded-[var(--radius)] border border-border">
          ...
          />
          {/* Overlay con intensidad configurable */}
          <div
            className={cn('absolute inset-0', centered ? 'bg-black' : 'bg-gradient-to-r from-black via-black/40 to-transparent')}
            style={{ opacity: (content.heroOverlay ?? 55) / 100 }}
          />
          <div className={cn('absolute inset-0 flex items-center', centered && 'justify-center text-center')}>
            <div className="container">
              <motion.div
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
                className={cn('max-w-xl text-white', centered && 'mx-auto')}
              >
                {content.seasonBadge && (
                  <span className="mb-3 inline-block rounded-full bg-brand-500 px-3 py-1 text-xs font-bold text-neutral-950 shadow-btn-brand">
                    {content.seasonBadge}
                  </span>
                )}
                ...
                <p className={cn('mt-4 max-w-md text-base text-white/85 sm:text-lg', centered && 'mx-auto')}>{content.heroSubtitle}</p>
                <div className={cn('mt-7 flex flex-col gap-3 sm:flex-row', centered && 'sm:justify-center')}>
                  <Link to={content.heroCtaLink || '/catalogo'}>
                    <Button size="lg" className="w-full sm:w-auto">{content.heroCtaText || 'Comprar ahora'} <ArrowRight className="h-4 w-4" /></Button>
                  </Link>
                  {content.heroSecondaryText && (
                    <Link to="/categorias/dama">
                      <Button size="lg" variant="secondary" className="w-full bg-white text-neutral-950 hover:bg-white/90 sm:w-auto">{content.heroSecondaryText}</Button>
                    </Link>
                  )}
                </div>
              </motion.div>
      ...
      {content.showCategories && (
        <section className="container py-14">
          <div className="mb-8 text-center">
            <h2 className="section-title">{content.categoriesTitle}</h2>
              ...
              <motion.div key={d.slug} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-60px' }} transition={{ duration: 0.5, delay: i * 0.08 }}>
                <Link to={`/categorias/${d.slug}`} className="group relative block aspect-[3/4] overflow-hidden rounded-3xl bg-muted">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 text-white">
                    <h3 className="font-display text-2xl font-extrabold tracking-tight">{d.label}</h3>
      ...
      {content.showTrending && (
        <section className="container py-6">
          <div className="mb-8 flex items-end justify-between">
            <div>
              <h2 className="section-title">{content.trendingTitle}</h2>
        ...
        <section className="container py-14">
          <div className="relative overflow-hidden rounded-3xl brand-gradient px-8 py-14 text-neutral-950 sm:px-14 sm:py-16">
            <div className="relative z-10 max-w-lg">
              ...
              <h3 className="mt-2 font-display text-3xl font-extrabold tracking-tight sm:text-4xl">{content.promoTitle}</h3>
      ...
      {/* ===== Novedades ===== */}
      {content.showNew && fresh.length > 0 && (
        <section className="container py-6">
          <div className="mb-8 flex items-end justify-between">
            <h2 className="section-title">{content.newTitle}</h2>
            <Link to="/catalogo" className="hidden items-center gap-1 text-sm font-semibold text-brand-700 hover:underline dark:text-brand-400 sm:flex">
              Ver todo <ChevronRight className="h-4 w-4" />
            </Link>
          </div>
      ...
      {/* ===== Reseñas teaser ===== */}
      {content.showReviewsHome && (
        <section className="container py-14">
          <div className="mb-8 text-center">
            <h2 className="section-title">Clientes felices</h2>
            <p className="mt-2 text-muted-foreground">Miles de personas ya confían en nosotros.</p>
          </div>
      ...
      {content.showNewsletter && <Newsletter content={content} />}
    </>
  );
}
  ...
  return (
    <section className="container py-16">
      <div className="mx-auto max-w-2xl rounded-3xl glass-panel p-8 text-center sm:p-12">
        <h3 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">{content.newsletterTitle}</h3>
